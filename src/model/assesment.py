...
class Assesment(Util.get_db().Model):
    ...
    teacher_id = Util.get_db().Column(Util.get_db().Integer, Util.get_db().ForeignKey('teachers.id'))
    ...
    def __init__(self, title, description, teacher_id, student_id, created_at, updated_at):
        self.title = title
